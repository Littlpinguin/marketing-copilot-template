...
- `06-graphic-design/presentations/templates/base.html` — the starter deck `:root` ships neutral graphite **values**, not `{{...}}` markers (a literal placeholder in CSS silently breaks the layout; the raw starter must pass `scripts/qa.py` as-is). **Replace the values** of the `:root` variables guided by the `← BRAND_*` comment next to each one: `--brand-primary` ← `BRAND_COLOR_PRIMARY`, `--brand-secondary` ← `BRAND_COLOR_ACCENT`, `--brand-neutral-light` / `--brand-neutral-dark` ← `BRAND_COLOR_LIGHT` / `BRAND_COLOR_DARK`, `--brand-gradient` ← `BRAND_GRADIENT`, and prepend `BRAND_FONT_PRIMARY` / `BRAND_FONT_SECONDARY` to the `--font-display` / `--font-mono` stacks. Also substitute `{{COMPANY_NAME}}` / `{{COMPANY_WEBSITE}}` in the `<title>` and chrome signatures. The other `{{DECK_*}}` / content placeholders are plain text and stay — the slides skill fills those per deck.

Read `_templates/brand/voice.md`, `_templates/brand/style-guide.md`, `_templates/brand/messaging-framework.md`. Substitute the validated values in place of `{{PLACEHOLDERS}}`. Write the result to `01-brand/<same-filename>.md`. Apply the same substitution pass to `06-graphic-design/presentations/tokens.css`, and update the `:root` values of `06-graphic-design/presentations/templates/base.html` as described above, so the slides skill is ready to inline a brand-aligned design system.
